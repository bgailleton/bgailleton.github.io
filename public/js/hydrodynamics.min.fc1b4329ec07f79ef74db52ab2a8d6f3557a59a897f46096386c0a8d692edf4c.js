const NX=256,NY=256,DT=.1,DX=10,GRAVITY=9.81,MANNING=.035,HFLOW_THRESHOLD=1e-4,FROUDE_LIMIT=.85,HILLSHADE_THRESHOLD=.1,DEFAULT_DEPTH_COLOR_SCALE=2.5,DEM_MIN=0,DEM_MAX=516,vertexShaderSrc=`
attribute vec2 aPos;
attribute vec2 aUV;
varying vec2 vUV;
void main() {
...
uniform vec2  uPointInflow;
uniform float uUsePointInflow;
...
void main() {
  ...
  if (uUsePointInflow > 0.5) {
    bool isPoint = abs(col - uPointInflow.x) < 0.5 && abs(row - uPointInflow.y) < 0.5;
    if (isPoint) {
      qx = uInflow;
      qy = 0.0;
    }
  } else if (col <= 0.5 && rowInInflow) {
    qx = -uInflow;
  }
