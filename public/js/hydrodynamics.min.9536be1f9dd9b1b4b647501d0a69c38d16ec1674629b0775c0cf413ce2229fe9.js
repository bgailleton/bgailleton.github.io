const NX=256,NY=256,DT=.5,DX=10,GRAVITY=9.81,MANNING=.035,HFLOW_THRESHOLD=1e-4,FROUDE_LIMIT=.85,HILLSHADE_THRESHOLD=.1,DEFAULT_DEPTH_COLOR_SCALE=2.5,DEM_MIN=0,DEM_MAX=516,SUN_ZENITH_RAD=20*Math.PI/180,SUN_AZIMUTH_RAD=315*Math.PI/180,vertexShaderSrc=`
attribute vec2 aPos;
attribute vec2 aUV;
varying vec2 vUV;
void main() {
...
float sampleDEM(float col, float row) {
  vec2 uv = (vec2(col, row) + 0.5) / uGridSize;
  return uZMin + uZScale * texture2D(uZ, vec2(uv.x, 1.0 - uv.y)).r;
}
