...
void main() {
  ...
  vec2 uvW = uv - vec2(uTexelSize.x, 0.0);
  vec2 uvE = uv + vec2(uTexelSize.x, 0.0);
  vec2 uvN = uv - vec2(0.0, uTexelSize.y);
  vec2 uvS = uv + vec2(0.0, uTexelSize.y);

  uvW.x = max(uvW.x, 0.5 * uTexelSize.x);
  uvE.x = min(uvE.x, 1.0 - 0.5 * uTexelSize.x);
  uvN.y = max(uvN.y, 0.5 * uTexelSize.y);
  uvS.y = min(uvS.y, 1.0 - 0.5 * uTexelSize.y);

  float h   = texture2D(uH, uv).r;
  float hW  = texture2D(uH, uvW).r;
  float hE  = texture2D(uH, uvE).r;
  float hN  = texture2D(uH, uvN).r;
  float hS  = texture2D(uH, uvS).r;

  float z   = uZMin + uZScale * texture2D(uZ, uv).r;
  float zW  = uZMin + uZScale * texture2D(uZ, uvW).r;
  float zE  = uZMin + uZScale * texture2D(uZ, uvE).r;
  float zN  = uZMin + uZScale * texture2D(uZ, uvN).r;
  float zS  = uZMin + uZScale * texture2D(uZ, uvS).r;

  vec2 Q = texture2D(uQ, uv).rg;
  float qx = Q.r;
  float qy = Q.g;

  float hflowY = max(z + h, zN + hN) - max(z, zN);
  if ((h > 0.0 || hN > 0.0) && (hflowY > uHFLOW_THRESHOLD)) {
    float surf   = z + h;
    float surfN  = zN + hN;
  ...
  float hflowX = max(z + h, zW + hW) - max(z, zW);
  if ((h > 0.0 || hW > 0.0) && (hflowX > uHFLOW_THRESHOLD)) {
    float surf  = z + h;
    float surfW = zW + hW;
