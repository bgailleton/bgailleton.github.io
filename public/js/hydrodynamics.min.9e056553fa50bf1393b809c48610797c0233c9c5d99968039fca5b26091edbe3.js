const NX=256,NY=256,DT=.5,DX=10,GRAVITY=9.81,MANNING=.035,HFLOW_THRESHOLD=1e-4,FROUDE_LIMIT=.85,HILLSHADE_THRESHOLD=.1,DEFAULT_DEPTH_COLOR_SCALE=2.5,DEM_MIN=0,DEM_MAX=516,vertexShaderSrc=`
attribute vec2 aPos;
attribute vec2 aUV;
varying vec2 vUV;
void main() {
...
vec3 hillshade(vec2 uv) {
  ...
  float dzdx = ((right - left) / (2.0 * uCellSize)) * uZExaggeration;
  float dzdy = ((up - down) / (2.0 * uCellSize)) * uZExaggeration;
  vec3 normal = normalize(vec3(-dzdx, -dzdy, 1.0));
  vec3 light = normalize(vec3(-0.2, 0.25, 2.75));
